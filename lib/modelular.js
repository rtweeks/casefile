...
export class Model {
  ...
  handleLockingAction(action, handler) {
    ...
    process.nextTick(() => {
      ...
      doStateUpdate.call(this, this[Private.state], {locked: true, skipResolve: true});
    });
  }
}
...
function doStateUpdate(newState, options = {}) {
  ...
  // When an internal call wants to skipResolve, skip the first use of Promise.resolve
  // so the resolution is totally synchronous
  const thenableNewState = (
    options.skipResolve
    ? ({then: fn => {
      try {
        return Promise.resolve(fn(newState));
      } catch (ex) {
  ...
  return thenableNewState.then(newState => {
