...
export function promiseToGetContentLines(path) {
  const contentLines = [];
  return promiseToRunGitCommand('show', {
...
export function promiseToShareCasefile(remote, path, bookmarks) {
  ...
  return promiseToRevParse(sharedCasefilesRef).then(
    ...
    // TODO: git commit-tree
    promiseToCommitCasefilesTree(newTree, {parents: parentCommits})
  ).then(newCommit =>
    Promise.all([
      promiseToUpdateRef(sharedCasefilesRef, newCommit),
      promiseToPush(remote, {source: newCommit, dest: sharedCasefilesRef})
    ])
  );
}
...
function promiseToMktree(entries) {
  ...
  return promiseToRunGitCommand('mktree', {
    args: ['-z'],
    operationDescription: 'building Git tree object',
    feedStdin: function(stdin) {
...
function promiseToCommitCasefilesTree(tree, {parents = []} = {}) {
  // Promises a commit hash
  const parentArgs = parents.flatMap(p => ['-p', p]);
  let result = null;
  return promiseToRunGitCommand('commit-tree', {
    args: parentArgs.concat(['-m', 'Sharing casefile', tree]),
...
//    * For each commit referenced by the casefile, use `git branch -r --contains`
//      to confirm the selected remote knows the commit:
//      * If not, offer to push a 'stake-in-the-ground' (invisible commit with
//       all commits in the casefile as parents)
...
//    * `git hash-object` of the JSON.stringify()'d  content -> new casefile entry
...
//    * `git commit-tree` from casefiles tree (using <parent-commits> (0 or 1) for parent(s)) -> new commit
//    * `git update-ref refs/collaboration/shared-casefiles <new-commit>`
//    * `git push <remote> refs/collaboration/shared-casefiles`
