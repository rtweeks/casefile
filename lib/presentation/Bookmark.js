...
function insertionFromPointInRect(point, rect) {
  ...
  try {
    var midlineY = (rect.top + rect.bottom) / 2, sameLevelX = (4 * rect.left + rect.right) / 5;
    var insert = {}; 
    if (point.y < midlineY) {
      return {siblingBefore: true};
    } else if (point.x < sameLevelX) {
...
class BookmarkBase {
  ...
  render() {
    ...
    BookmarkBody = DropTarget(
      ...
      (monitor) => {
        var targRect = monitor.targetBounds(), pointerAt = monitor.pointerCoords();
        var insert = pointerAt && monitor.canDrop() && monitor.isOver() && 
          insertionFromPointInRect(pointerAt, targRect);
        if (insert && monitor.getTargetElement()) {
          ...
          if (insert.siblingBefore) {
            ...
            markLocation = {left: targRect.left, y: targRect.bottom};
          } else if (insert.child) {
            let $handle = jQuery(monitor.getTargetElement()).find('.drag-handle');
          ...
          if (markLocation) {
            let markRect = {
              ...
              width: targRect.width - markLocation.left + "px",
              right: "",
              bottom: ""
            };
