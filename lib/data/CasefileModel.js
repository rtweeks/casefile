...
import { validFile, readPersisted, derelativizePaths } from '../persisted';
...
class CasefileModel extends Model {
  ...
  getMarkInfo(itemPath) {
    const markPath = getMarkPath(this.getState(), itemPath);
...
function promiseToReadBookmarks(path, options = {}) {
  return promiseToGetContentLines(path, options).then(contentLines => {
    if (contentLines[0] && (contentLines[0].startsWith('[') || contentLines[0].startsWith('{'))) {
      try {
        let jsonValue = JSON.parse(contentLines.join(' '));
        if (!('length' in jsonValue)) {
          jsonValue = jsonValue.bookmarks;
        }
        return derelativizePaths(jsonValue);
      } catch (ex) {
        // try next
      }
    }
    ...
    throw `Invalid casefile content in '${path}'`;
  });
}
...
action(AType.IMPORT_SHARED_FILE, (state, action) => {
  ...
  if (multiPath || state.length > 0) {
    ...
    return promiseToReadBookmarks(action.paths[0]);
  }
});

action(AType.RECOVER_SHARED_FILE, (state, action) => {
  const bookmarkPromise = promiseToReadBookmarks(action.path, {
    beforeCommit: action.commit
  });
  if (state.length === 0) {
    return bookmarkPromise;
  }
  return bookmarkPromise.then(bookmarks => {
    state = state.slice();
    state.push({
      file: null,
      line: null,
      markText: `Recovered "${action.path.split('/', 1)[0]}" casefile`,
      notes: '',
      children: bookmarks,
      id: nextId()
    });
